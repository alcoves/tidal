...
app.post("/segments/:videoId/:sourceSegmentDir/:width/:segment", (req, res) => {
  const { videoId, sourceSegmentDir, width, segment } = req.params;

  s3.upload({
    Body: req,
    Bucket: BUCKET,
    Key: `segments/${videoId}/${sourceSegmentDir}/${segment}`,
  })
    .promise()
    .then((data) => {
      const messages = getPresets(width).map(({ presetName, ffmpegCmdStr }) => {
        return {
          ...
          MessageBody: JSON.stringify({
            ...
            in_path: `s3://${BUCKET}/segments/${videoId}/source/${segment}`,
            out_path: `s3://${BUCKET}/segments/${videoId}/${presetName}/${segment}`,
          }),
        };
      });
