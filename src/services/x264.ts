...
import s3, { defaultBucket, getSignedURL } from '../config/s3'
...
async function optimizeSourceVideo(video: Video, command: string) {
  const signedUrl = await getSignedURL({
    Bucket: defaultBucket,
    Key: `v/${video.id}/original`,
  })
  const tmpDir = await fs.mkdtemp('/tmp/tidal-')
  const filePath = `${tmpDir}/optimized.mp4`

  return new Promise((resolve, reject) => {
    ffmpeg(signedUrl)
      .outputOptions(command.split(' '))
      .output(filePath)
      ...
      .on('progress', async function (progress) {
        ...
        if (progress.percent) {
          await db.video.update({
            where: { id: video.id },
            data: { progress: progress.percent, status: 'TRANSCODING' },
          })
        }
      })
...
export async function optimizeOriginal(videoId: string) {
  ...
  if (video.width > 1920) {
    command += `-vf scale=${1920}:${1920}:force_original_aspect_ratio=decrease,scale=trunc(iw/2)*2:trunc(ih/2)*2`
  }

  // optimize original video
  await optimizeSourceVideo(video, command)
}
