...
class Conversion {
  constructor({ videoId, sourcePath, thumbnail = false, skipDelete = false }) {
    console.log('the source path is', sourcePath);
  ...
  async process(fileName) {
    try {
      ...
      if (!this.thumbnail) {
        console.log('reporting startedAt time');
        await request({
          method: 'patch',
      ...
      if (this.skipDelete) {
        console.log(
          'skipping removal and returning',
          `${this.outPath}/highQuality.mp4`
        );
        return { sourcePath: `${this.outPath}/highQuality.mp4` };
      } else {
        console.log('removing ffmpeg files from disk');
