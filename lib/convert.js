...
const streamingProm = (cmd, meta) => {
  return new Promise((resolve, reject) => {
    const cmdStream = spawn(cmd);

    cmdStream.stdout.on('data', (data) => {
      console.log(`cmdStream stdout:\n${data}`);
    });

    cmdStream.stderr.on('data', (data) => {
      console.error(`cmdStream stderr:\n${data}`);
    });

    cmdStream.on('disconnect ', (data) => {
      console.error(`disconnect :\n${data}`);
    });

    cmdStream.on('close ', (data) => {
      console.error(`close :\n${data}`);
